...
fn test_empty_input() {
    ...
    assert!(pending.is_empty(), "Empty input should have no pending bytes");
    assert!(chunker.finish().is_none(), "finish() on empty state should return None");
}
...
fn test_small_data_below_min_size() {
    ...
    assert!(chunks.is_empty(), "Data below min_size should not produce chunks");
    assert_eq!(pending.len(), 3, "All data should be pending");
    
    let final_chunk = chunker.finish().expect("finish() should emit pending data");
    assert_eq!(final_chunk.len(), 3, "Final chunk should contain all pending data");
}
...
fn test_data_at_min_size_boundary() {
    ...
    assert!(chunks.is_empty() || pending.is_empty(), 
...
fn test_large_data_finds_boundaries() {
    ...
    assert!(!all_chunks.is_empty(), "Large data should produce at least one chunk");
    
    let total_output: usize = all_chunks.iter().map(|c| c.len()).sum();
    assert_eq!(total_output, data.len(), "Output bytes must match input bytes");
}
...
fn test_pending_bytes_handling() {
    ...
    assert!(!chunks2.is_empty() || !pending2.is_empty(), 
...
fn test_multiple_finish_calls() {
    ...
    assert!(final1.is_none(), "First finish() after push should return None (already emitted)");
...
fn test_offset_resets_after_reset() {
    ...
    let (chunks, _) = chunker.push(Bytes::from(&b"first"[..]));
    chunker.finish();
    assert!(chunker.offset() > 0, "Offset should be > 0 after processing");
    ...
    assert_eq!(all.first().unwrap().offset, Some(0), 
...
fn test_max_size_enforces_boundary() {
    ...
    assert!(chunks[0].len() <= 8, "First chunk should not exceed max_size");
}
...
fn test_exact_max_size_boundary() {
    ...
    assert!(all_chunks[0].len() <= 32, "Chunk should not exceed max_size");
}
...
fn test_determinism_across_push_sizes() {
    ...
    let offsets1: Vec<_> = chunks1.iter()
        .chain(final1.iter())
    ...
    for chunk in data.chunks(10) {
        let (chunks, _leftover) = chunker2.push(Bytes::from(chunk.to_vec()));
        chunks2.extend(chunks);
    }
    let final2 = chunker2.finish();
    let offsets2: Vec<_> = chunks2.iter()
        .chain(final2.iter())
        .map(|c| c.offset.unwrap())
        .collect();
...
fn test_same_stream_same_chunks_same_hashes() {
    ...
    for chunk in data.chunks(37) {
        let (chunks, _leftover) = chunker3.push(Bytes::from(chunk.to_vec()));
        all3.extend(chunks);
    }
...
mod hashing_tests {
    ...
    fn test_hashing_enabled() {
        ...
        for (i, chunk) in chunks.iter().chain(final_chunk.iter()).enumerate() {
            assert!(chunk.hash.is_some(), "Chunk {} must have a hash when enabled", i);
        }
    }
    ...
    fn test_hashing_disabled() {
        ...
        for (i, chunk) in chunks.iter().chain(final_chunk.iter()).enumerate() {
            assert!(chunk.hash.is_none(), "Chunk {} must not have a hash when disabled", i);
        }
    }
    ...
    fn test_hash_persists_across_chunks() {
        let config = ChunkConfig::new(4, 8, 16).unwrap()
            .with_hash_config(HashConfig::enabled());
        let mut chunker = Chunker::new(config);
        ...
        for (i, chunk) in all_chunks.iter().enumerate() {
            assert!(chunk.hash.is_some(), 
                   "All chunks (including {}) must have hashes when enabled", i);
            // Different chunks should generally have different hashes
            if i > 0 {
                assert_ne!(chunk.hash, all_chunks[i-1].hash,
...
fn test_config_validation() {
    // Invalid: min > avg
    assert!(ChunkConfig::new(16, 8, 64).is_err(), 
            "min > avg should be invalid");
    
    // Invalid: avg > max
    assert!(ChunkConfig::new(4, 32, 16).is_err(), 
            "avg > max should be invalid");
    
    // Invalid: zero sizes
    assert!(ChunkConfig::new(0, 16, 64).is_err(), 
...
fn test_hash_config_consistency() {
    ...
    let config1 = ChunkConfig::new(4, 16, 64).unwrap()
        .with_hash_config(HashConfig::enabled());
    let config2 = ChunkConfig::new(4, 16, 64).unwrap()
        .with_hash_config(HashConfig::enabled());
    ...
    assert_eq!(all1.len(), all2.len(), "Same config should produce same number of chunks");
