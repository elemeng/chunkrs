//! FastCDC lookup tables.

/// Base gear hash table for FastCDC.
...
pub const GEAR_TABLE_SHIFTED: [u64; 256] = {
    ...
    let mut i = 0u32;
    while i < 256 {
        shifted[i as usize] = GEAR_TABLE[i as usize].wrapping_shl(1);
        i += 1;
    }
    shifted
};
...
pub const MASKS: [u64; 32] = [
    ...
    0x0000_0000_d903_0353, // 2^13 (8KB)
    0x0000_0001_b207_06a7, // 2^14 (16KB)
    0x0000_0000_3590_7035, // 2^15 (32KB)
...
/// Generates a keyed gear table using BLAKE3 (requires `keyed-cdc` feature).
#[cfg(feature = "keyed-cdc")]
pub fn generate_keyed_gear_table(key: [u8; 32]) -> [u64; 256] {
    use blake3::Hasher;

    let mut keyed = [0u64; 256];
    for i in 0..256 {
        let mut hasher = Hasher::new_keyed(&key);
        hasher.update(&GEAR_TABLE[i].to_le_bytes());
...
/// Generates a pre-shifted keyed gear table (requires `keyed-cdc` feature).
#[cfg(feature = "keyed-cdc")]
pub fn generate_keyed_gear_table_shifted(key: [u8; 32]) -> [u64; 256] {
